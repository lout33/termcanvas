...
test("canvas header merges global context and canvas status into one row", () => {
  ...
  // Terminal tabs moved into the sidebar as the terminal-navigator section.
  assert.doesNotMatch(html, /terminal-strip-topbar-section/i);
  assert.match(html, /<section class="sidebar-section sidebar-primary-section sidebar-view-panel" id="terminal-navigator-section"/i);
  assert.match(html, /<div class="sidebar-view-switcher" id="sidebar-view-switcher"/i);
});

test("sidebar view switcher exposes explorer and terminals tabs", () => {
  const html = readIndexHtml();

  assert.match(html, /<button class="sidebar-view-tab"[^>]*data-sidebar-view="explorer"/i);
  assert.match(html, /<button class="sidebar-view-tab"[^>]*data-sidebar-view="terminals"/i);
});
